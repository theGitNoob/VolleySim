...
class Player:
    ...
    def takeAction(self,actionList:list,ball:tuple[int,int]):
        random_index=random.randint(0,len(actionList))
        action_to_make= actionList[random_index] #escoge una accion al azar de una lista de acciones qe se le pase
        action_to_make(self,ball)
        

class Actions:
    @staticmethod
    def pass_ball(player: Player,ball:tuple[int,int]):
    ...
    @staticmethod
    def block_ball(player: Player,ball:tuple[int,int]):
    ...
    @staticmethod
    def shoot_ball(player: Player,ball:tuple[int,int]):
